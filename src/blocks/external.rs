...
//! output pin, and [`ExternalOut`] publishes input pin values through a
//! connector. Both resolve their connector by name from the process-wide
//! registry, so the same program works with any protocol adapter that is
