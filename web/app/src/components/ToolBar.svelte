<script lang="ts">
  ...
  let { blocks, onAddBlock, onReset, onCopy, onPaste, onLoad }: Props =
    $props();
  ...
  const blocksFiltered = $derived(
    blocks.filter((b) =>
      b.dis.toLowerCase().includes(blockSearch.toLowerCase()),
    ),
  );
...
<div
  class="flex flex-wrap items-center justify-center gap-2 rounded-lg border bg-background px-3 py-2 shadow-md max-w-[calc(100dvw-1rem)]"
>
        ...
        {#each blocksFiltered as block (block.lib + '/' + block.name)}
          <DropdownMenu.Item onclick={() => onAddBlock(block)}>
            ...
            <span class="ml-auto text-xs text-muted-foreground"
              >{capitalize(block.category)}</span
            >
          </DropdownMenu.Item>
        {/each}
        ...
        {#each categories as category, i (category)}
          ...
          <DropdownMenu.Sub>
            <DropdownMenu.SubTrigger
              >{capitalize(category)}</DropdownMenu.SubTrigger
            >
